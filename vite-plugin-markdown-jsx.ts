  ...
  if (options.mode?.includes(Mode.REACT)) {
    ...
    const compiledReactCode = `
      function (props) {
        ...
        ${(await import('@babel/core')).transformSync(reactCode, { ast: false, presets: [ ['@babel/preset-react', {pragma: "h", pragmaFrag: "Fragment"}]] })?.code}
        return markdown
      }
    `
    content.addContext(`const ${subComponentNamespace} = {}\nconst ReactComponent = ${compiledReactCode}`)
